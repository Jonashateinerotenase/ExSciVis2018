...
//taking the mean by deviding the max value
  ...
  for (size_t d = 0; d < (344); ++d) 
  {
    for (size_t h = 0; h < (523); ++h) 
    {
      for (size_t w = 0; w < (504); ++w) 
      {
        ...
        {
          if(d % GRIDFINENESS == i)value3D = ((float)i/(float)depth) * (float)addup[(int)(w / GRIDFINENESS)][(int)(h / GRIDFINENESS)][(int)(d / GRIDFINENESS)] + (1.0 - (float)i/(float)depth) * (float)addup[(int)(w / GRIDFINENESS)][(int)(h / GRIDFINENESS)][(int)(d / GRIDFINENESS) - 1];
          if(h % GRIDFINENESS == i)value3D = ((float)i/(float)depth) * (float)addup[(int)(w / GRIDFINENESS)][(int)(h / GRIDFINENESS)][(int)(d / GRIDFINENESS)] + (1.0 - (float)i/(float)depth) * (float)addup[(int)(w / GRIDFINENESS)][(int)(h / GRIDFINENESS) - 1][(int)(d / GRIDFINENESS)];
          if(w % GRIDFINENESS == i)value3D = ((float)i/(float)depth) * (float)addup[(int)(w / GRIDFINENESS)][(int)(h / GRIDFINENESS)][(int)(d / GRIDFINENESS)] + (1.0 - (float)i/(float)depth) * (float)addup[(int)(w / GRIDFINENESS) - 1][(int)(h / GRIDFINENESS)][(int)(d / GRIDFINENESS)];
        }
  ...
  std::ofstream out("ConcreteSMTH_w504_h523_d344_c1_b8.raw", std::ios::binary);
